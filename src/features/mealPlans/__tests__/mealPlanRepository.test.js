...
describe('meal plan persistence', () => {
  ...
  it('upgrades v1 without changing existing ingredients', async () => {
    ...
    const upgraded = await openRaw(2);
    expect(Array.from(upgraded.objectStoreNames)).toEqual(['ingredients', 'mealPlans']);
    upgraded.close();
  });
  ...
  it('account deletion clears that account’s ingredients and plans but preserves guests', async () => {
    const repository = await import('../mealPlanRepository.js');
    const db = await import('../../../db/indexedDB.js');
