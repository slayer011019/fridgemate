...
function RecipesPage() {
  ...
  const { recommendations, loading, ingredients } = useRecipeRecommendations(pantryOwnership);
  ...
  const recommendationCoverage = useMemo(
    () => recommendations.ready.length + recommendations.buyOne.length,
    [recommendations.buyOne.length, recommendations.ready.length]
  );
  ...
  const sectionStats = useMemo(
    () => ({
      ready: {
        value: loading ? '...' : recommendations.ready.length,
        helper: loading
          ? '추천을 정리 중이에요'
          : getSectionHelperText(
              recommendations.ready.length,
              '아직 없어요',
              '바로 해볼 수 있어요',
              '오늘 바로 고를 수 있어요'
            )
      ...
      buyOne: {
        value: loading ? '...' : recommendations.buyOne.length,
        helper: loading
          ? '추천을 정리 중이에요'
          : getSectionHelperText(
              recommendations.buyOne.length,
              '재료가 조금 더 필요해요',
              '한 두 개만 채우면 돼요',
              '장보기 효율이 좋아요'
            )
      ...
      other: {
        value: loading ? '...' : recommendations.other.length,
        helper: loading
          ? '추천을 정리 중이에요'
          : getSectionHelperText(
              recommendations.other.length,
              '아직 후보가 적어요',
              '먼저 처리할 재료가 보여요',
              '소비 순서를 잡기 좋아요'
            )
    ...
    [loading, recommendations.buyOne.length, recommendations.other.length, recommendations.ready.length]
  );
