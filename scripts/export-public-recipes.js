...
export function parseArgs(argv = process.argv.slice(2)) {
  ...
  if (writeFromArg && !writeFromArg.slice('--write-from='.length).trim()) {
    throw new Error('--write-from requires a reviewed local JSON file.');
  }
  if (writeFromArg && argv.includes('--print-review')) {
    throw new Error('Choose either a network preview or a reviewed local file import.');
  }
...
async function fetchRows(apiKey, limit) {
  if (!Number.isInteger(limit) || limit < 1 || limit > MAX_LIMIT) {
    throw new Error(`The preview limit must be an integer between 1 and ${MAX_LIMIT}.`);
  }
  const url = `${API_BASE_URL}/${encodeURIComponent(apiKey)}/${DATASET_ID}/json/1/${limit}`;
  ...
  const rows = Array.isArray(payload?.[DATASET_ID]?.row) ? payload[DATASET_ID].row : [];
  if (rows.length > limit) {
    throw new Error('Food Safety Korea returned more rows than the preview limit.');
  }
  return rows;
}
...
export async function writeReviewedPublicRecipes(reviewFile, { workspaceRoot = process.cwd() } = {}) {
  const rows = await readReviewedRows(reviewFile, { workspaceRoot });
  const recipes = rows.map(mapPublicRecipe).filter(Boolean);

  await writeVerifiedPublicRecipeArtifact(
    resolve(workspaceRoot, 'src/data/publicRecipes.json'),
...
export async function exportPublicRecipes(options = parseArgs()) {
  ...
  if (options.writeFrom && options.printReview) {
    throw new Error('Choose either a network preview or a reviewed local file import.');
  }
