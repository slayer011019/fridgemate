...
- Continue from the verified sixth 25-row missing batch at `recipes=1,166`, `embeddings=1,153`, `current=170`, `missing=13`, and `stale=983`; the final missing batch remains separately approved and capped, while the fixed ten-query stored-vector gate remains Hit@1 `9/10` and Hit@5 `10/10`
