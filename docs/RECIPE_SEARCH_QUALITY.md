...
8. Replace stale rows in separately approved batches, then verify total count 1,166, dimensions 1,536, duplicate keys 0, orphans 0, and re-run both stored-vector quality fixtures.
9. Roll back by restoring the protected `recipe_embeddings` snapshot if stored-vector quality regresses.

The in-memory, missing-row coverage, limited stale-row, integrity, and stored-vector quality gates are satisfied. Every catalog recipe now has a stored vector. The next separately reviewed operation is a checkpointed, capped replacement of 25 stale rows followed by a stored-vector quality rerun. Full freshness is not yet achieved, and semantic API release remains blocked until all 983 stale rows are replaced, integrity checks pass, and both stored-vector fixtures are rerun. See `docs/RECIPE_EMBEDDING_OPERATIONS.md` for the production record.
